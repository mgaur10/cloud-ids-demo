    ...
    */
    ...
    provisioner "local-exec" {
        command     =  "gcloud ids endpoints create cloud-ids-${var.vpc_network_name} --network=${var.vpc_network_name} --zone=${var.network_zone} --severity=INFORMATIONAL --no-async --project ${var.demo_project_id}${random_string.id.result}"
    }
    ...
    provisioner "local-exec" {
    command     =  <<EOT
    echo "${var.demo_project_id}${random_string.id.result}" >> proj_id-${random_string.id.result}.txt
    EOT
    ...
    provisioner "local-exec" {
    command     =  <<EOT
    gcloud ids endpoints describe cloud-ids-${var.vpc_network_name} --zone=${var.network_zone} --project ${var.demo_project_id}${random_string.id.result} --format="value(endpointForwardingRule)" >> f_rule-${random_string.id.result}.txt
    EOT
    working_dir = path.module
    }
    ...
    filename = "f_rule-${random_string.id.result}.txt"
    depends_on = [null_resource.forward_rule]
    }

    # data file for project id
    data "local_file" "proj_id" {
    filename = "proj_id-${random_string.id.result}.txt"
    ...
    provisioner "local-exec" {
    command     =  <<EOT
    gcloud compute packet-mirrorings create cloud-ids-packet-mirroring --region=${var.network_region} --network=${var.vpc_network_name} --mirrored-subnets=ids-network-${var.network_region} --project=${var.demo_project_id}${random_string.id.result} --collector-ilb=${data.local_file.forward_rule.content}
    EOT
    #    working_dir = path.module
    }
